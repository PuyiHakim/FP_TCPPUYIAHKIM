	...
	DWORD WINAPI Receive (LPVOID Param)
{
	...
	{
		cout << endl << "Koneksi Gagal terima" << endl;
	}
	else
	{
		cout << endl << "Koneksi Berhasil Terima" << endl << "send";
	}
	while (true)
	{
		Sleep(100);
		...
		{
			cout << endl << "pesan atau data dari pengrimin : " << szMessage << endl << "send";
		}
	}
}
DWORD WINAPI Connect (LPVOID Param)
{
	...
	{
		Sleep(1000);
		bKondisi = bConnectTo("192.168.0.15", 2014);
		if (bKondisi == true)
		{
			cout << endl << "Connection Suckess" << endl << "send";
			break;
		}
	}
	return 0;
